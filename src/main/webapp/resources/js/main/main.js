	...
	for (var i = 0; i < tabBtnList1.length; i++) {
		tabBtnList1[i].addEventListener('click', function (e) {
			...
			if ($(this).attr("href") == "#noticeTab") {
				moreInfo_L.setAttribute("href", "./customer/cusAnncmList");
			} else {
				moreInfo_L.setAttribute("href", "./customer/faqPage");
			}
...
function setAnncm(data) {
	...
	anncmName.setAttribute("href", "./customer/cusReadAnncm?anncm_no=" + data.ANNCM_NO);
...
function setFaQ(data) {
	...
	faqName.setAttribute("href", "./customer/faqContentPage?faq_no=" + data.FAQ_NO);
